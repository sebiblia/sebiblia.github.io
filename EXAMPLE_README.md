...
2. Spaces are replaced by hyphens (-). Any other whitespace or punctuation characters are removed.
3. Leading and trailing whitespace are removed.
4. Markup formatting is removed, leaving only the contents (for example, _italics_ becomes italics).
5. If the automatically generated anchor for a heading is identical to an earlier anchor in the same document, a unique identifier is generated by appending a hyphen and an auto-incrementing integer.
